...
def check_database_updates():
    ...
    try:
        with app.app_context():
            ...
            # 檢查是否有新的資料插入
            if max_id > last_message_time:
                last_message_time = max_id
                # 執行查詢，按主鍵排序
                sql_cmd = text("""SELECT 主鍵, number FROM test WHERE 主鍵 > :last_id ORDER BY 主鍵 ASC""")
                result = db.session.execute(sql_cmd, {"last_id": last_message_time})
